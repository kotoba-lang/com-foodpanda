(ns foodpanda.main
  "Kotodama WASM entrypoint for the Foodpanda clean-room actor (L4) — Clojure port.
  ...
  facts (`foodpanda.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "foodpanda")
(def tier "L4")
...
(def entity-specs
  [{:entity "Order"      :plural "orders"      :id-prefix "foodpand_ord"
   ...
   {:entity "Tracking"   :plural "trackings"   :id-prefix "foodpand_tra"
...
(defn emit-facts
  "EAVT facts for one record: {\"foodpanda.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "foodpanda-compat" :tier tier :entities entities} 200])
...
(defn start! [] :foodpanda-compat/ready)
